...
def main():
    parser=argparse.ArgumentParser(); parser.add_argument("--config",default="config.yaml"); args=parser.parse_args(); cfg=load_config(args.config); tc=cfg["training"]
    # Create the complete output tree before loading data/model so every run is self-contained.
    output=Path(tc["output_dir"]); output.mkdir(parents=True,exist_ok=True)
    seed_everything(tc["seed"]); data=load_iemocap(cfg["data"]["path"]); tokenizer,token_ids=build_tokenizer(cfg["model"]["backbone"]); model=build_model(cfg,tokenizer,token_ids,data["train"][0],True); print(json.dumps(model.parameter_report(),indent=2)); optimizer=torch.optim.AdamW([p for p in model.parameters() if p.requires_grad],lr=tc["learning_rate"],weight_decay=tc["weight_decay"]); best=-1.; history=[]
