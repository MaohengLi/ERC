...
def main():
    ...
    with torch.no_grad():
        for x in d["test"]:
            b=collate_dialogues([x],tok,ids,a.max_length); y+=b["labels"].tolist(); b={k:(v.cuda() if isinstance(v,torch.Tensor) else v) for k,v in b.items()}; pred+=m(b)["logits"].argmax(-1).cpu().tolist()
