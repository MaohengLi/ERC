...
def main():
    ...
    with torch.no_grad():
        for x in d["test"]:
            initial=collate_dialogues([x],tok,ids,a.max_length,standard=True); initial={k:(v.cuda() if isinstance(v,torch.Tensor) else v) for k,v in initial.items()}; out=m(initial,initial["labels"],tokenizer=tok,dialogues=[x],max_length=a.max_length); y+=initial["labels"].tolist(); pred+=out["logits"].argmax(-1).cpu().tolist()
