...
from utils.prompt import classify_dialogue
...
def main():
    ...
    data=load_iemocap(a.data_path); train=data["train"][:a.max_train_dialogues or None]; tok,ids=build_tokenizer(a.model_name); s=train[0]; model=CausalERCQwen3(load_qwen3_4b(a.model_name,len(tok),True),ids,len(s["text_feats"][0]),len(s["audio_feats"][0]),len(s["visual_feats"][0]),lambda_hgr=a.lambda_hgr).cuda(); print(json.dumps(model.parameter_report(),indent=2)); opt=torch.optim.AdamW([x for x in model.parameters() if x.requires_grad],lr=a.learning_rate); cache={}; best=-1.; history=[]
    for epoch in range(1,a.epochs+1):
        model.train(); current={}; y=[]; pred=[]; total=0.
        for step in range(0,len(train),a.batch_size):
            ds=train[step:step+a.batch_size]; decisions={k:classify_dialogue(v,a.history_window) for k,v in cache.items()} if cache else None; b=collate_dialogues(ds,tok,ids,a.max_length,a.history_window,decisions); b={k:(v.cuda() if isinstance(v,torch.Tensor) else v) for k,v in b.items()}; o=model(b,b["labels"]); (o["loss"]/a.grad_accumulation).backward(); total+=float(o["loss"]); pred+=o["logits"].argmax(-1).detach().cpu().tolist(); y+=b["labels"].cpu().tolist(); off=0
            for d in ds: current[d["vid"]]=o["logits"][off:off+d["utt_count"]].detach().cpu(); off+=d["utt_count"]
            if ((step//a.batch_size)+1)%a.grad_accumulation==0 or step+a.batch_size>=len(train): opt.step(); opt.zero_grad(set_to_none=True)
        cache=current; m=classification_metrics(y,pred); rec={"epoch":epoch,"loss":total/max(1,len(train)),**m}; history.append(rec); print(json.dumps(rec),flush=True)
