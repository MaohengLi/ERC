...
class DialogueRNN(nn.Module):
    ...
    def forward(self,x,speaker_ids,lengths=None):
        B,T,_=x.shape; q=x.new_zeros(B,self.speaker.hidden_size); c=q.clone(); e=q.clone(); out=[]
        for t in range(T):
            active=torch.ones(B,dtype=torch.bool,device=x.device) if lengths is None else t<lengths.to(x.device)
            z=self.norm(x[:,t]+self.spk_emb(speaker_ids[:,t])); qn=self.speaker(self.drop(z),q); cn=self.context(qn,c); en=self.emotion(torch.cat([qn,cn],-1),e)
            q=torch.where(active[:,None],qn,q); c=torch.where(active[:,None],cn,c); e=torch.where(active[:,None],en,e); out.append(e)
